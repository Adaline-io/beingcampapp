...
- `supabase/` — schema migrations (24 tables), RLS policies, atomic wallet
  RPC (`record_transaction`, with per-txn + daily earn caps), dispatch
  marketplace (rank-gated crew seats, real escrow funding, 85/15 split to
  crew/house treasury, claim/unclaim/kick, cancel+refund, client tokens),
  reliability (`member_stats`), notification triggers + realtime, seed catalog. Live project: `beingcampapp`
  (eutcbcbalfnwgqddhdog, keys committed in `src/lib/config.ts` — the anon key
  is publishable by design, protected by RLS).
- `tests/e2e/` — 25 Playwright tests covering every core flow; CI runs them
