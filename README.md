...
- **Capacitor 6** — native iOS & Android apps from the same codebase (`ios/`, `android/`)
- **PWA** — installable, offline-capable web app (service worker + manifest)
- Deploys to **GitHub Pages** _or_ **Vercel** (static output, deploy anywhere)

## One codebase, three platforms

The same build ships everywhere, and the app adapts automatically:

| Where it runs                                  | What you get                                                     |
| ---------------------------------------------- | ---------------------------------------------------------------- |
| **Phone browser / installed PWA**              | Full-screen native-feeling app (edge-to-edge, safe-area aware)   |
| **Native iOS / Android** (Capacitor shells)    | Real app-store apps wrapping the same build                     |
| **Desktop browser**                            | Centered phone presentation of the product                      |

Platform detection lives in the root component: native shell / standalone PWA /
touch-phone viewport → full-bleed; otherwise the desktop presentation. Design
tweaks (accent, rank, demo state) are development chrome — desktop-only, behind
`?debug` in the URL.
...
npx cap open android    # open in Android Studio → run/build APK or AAB
npx cap open ios        # open in Xcode (macOS) → run/build for App Store
```

The native projects are committed (`android/`, `ios/`); the web assets inside
them are gitignored and regenerated by `cap sync`. App id: `com.adaline.beingcamp`.
...
| `npm run cap:sync`  | Build web + sync into the native iOS/Android shells  |
| `npm run cap:android` | Sync + open in Android Studio                      |
| `npm run cap:ios`   | Sync + open in Xcode (macOS)                         |
